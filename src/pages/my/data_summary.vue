<template>
  <view class="container">
    <view class="card">
      <text class="title">数据小结</text>

      <view class="section">
        <text class="section-title">📊 计划进度模块</text>
        <view class="grid">
          <view class="item"><text class="label">使用天数</text><text class="value">{{ progress.useDays }}</text></view>
          <view class="item"><text class="label">记录天数</text><text class="value">{{ progress.recordDays }}</text></view>
          <view class="item"><text class="label">坚持方案</text><text class="value">{{ progress.planDays }}</text></view>
          <view class="item"><text class="label">剩余天数</text><text class="value">{{ progress.remainingDays }}</text></view>
        </view>
        <view class="status-row">
          <text>{{ progress.avatarEmoji }} {{ progress.avatarDesc }}</text>
          <text>BMI {{ progress.bmi || '--' }}</text>
        </view>
      </view>
      ...
      <view class="section">
        ...
        <view class="row"><text>记录条数</text><text>{{ weight.count }} 条</text></view>
      </view>
...
.title { display: block; font-size: 34rpx; font-weight: 700; color: #4FA1F2; margin-bottom: 16rpx; }
.section { margin-bottom: 16rpx; padding: 14rpx; border: 1rpx solid #eef4fb; border-radius: 12rpx; }
.section-title { display:block; font-size: 28rpx; font-weight: 600; color:#333; margin-bottom: 10rpx; }
.grid { display:grid; grid-template-columns: 1fr 1fr; gap: 8rpx; }
.item { background:#f6faff; border-radius:10rpx; padding: 10rpx; text-align:center; }
.label { display:block; font-size:22rpx; color:#888; }
.value { display:block; font-size:30rpx; font-weight:700; color:#4FA1F2; }
.row { display:flex; justify-content:space-between; font-size:26rpx; color:#333; padding: 8rpx 0; border-bottom: 1rpx dashed #eef2f6; }
.row:last-child { border-bottom:none; }
.status-row { margin-top:8rpx; display:flex; justify-content:space-between; font-size:24rpx; color:#666; }
</style>
